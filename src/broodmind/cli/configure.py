...
from broodmind.infrastructure.config.settings import load_config, save_config, _resolve_env_file, Settings
from broodmind.infrastructure.providers.catalog import (
    get_provider_catalog_entry,
    list_registered_provider_ids,
)
from broodmind.channels.whatsapp.ids import normalize_whatsapp_number
...
def configure_wizard() -> None:
    ...
    if env_file and env_file.exists() and not Path("config.json").exists():
        if Confirm.ask("[yellow]Found legacy .env but no config.json. Migrate now?[/yellow]", default=True):
            save_config(config)
...
def _configure_user_channel(config: BroodMindConfig, advanced: bool) -> None:
    ...
    channel = Prompt.ask(
        ...
        default=config.telegram.bot_token and "telegram" or "whatsapp",
    )
    
    # We should really have a better way to set user_channel in BroodMindConfig
    # For now, we'll infer it or add it to the model if needed. 
    # Since it's not in BroodMindConfig currently (it was in Settings), 
    # we'll assume the existence of tokens/configs determines it.
