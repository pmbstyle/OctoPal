...
def config_audit(args: dict[str, Any], ctx: dict[str, Any]) -> str:
    ...
    required = ["TELEGRAM_BOT_TOKEN", "OCTOPAL_LLM_PROVIDER", "ALLOWED_TELEGRAM_CHAT_IDS"]
    ...
    try:
        ...
        present.setdefault("OCTOPAL_LITELLM_PROVIDER_ID", False)
        present.setdefault("OCTOPAL_LITELLM_API_KEY", False)
        present.setdefault("OCTOPAL_LITELLM_MODEL", False)
    ...
    return _json(
        {
            ...
            "has_litellm_provider_id": bool(present.get("OCTOPAL_LITELLM_PROVIDER_ID", False)),
            "has_litellm_key": bool(present.get("OCTOPAL_LITELLM_API_KEY", False)),
            "has_litellm_model": bool(present.get("OCTOPAL_LITELLM_MODEL", False)),
...
def _state_dir() -> Path:
    p = Path(os.getenv("OCTOPAL_STATE_DIR", "data")).resolve()
...
def _resolve_db_path(args: dict[str, Any], ctx: dict[str, Any]) -> Path | str:
    ...
    db = Path(db_path_raw).resolve() if db_path_raw else (_state_dir() / "octopal.db").resolve()
    if not db.exists():
...
def _allowed_services() -> set[str]:
    raw = os.getenv("OCTOPAL_ALLOWED_SERVICES", "octopal,ast,tts,infer,translator")
