...
def get_skill_management_tools() -> list[ToolSpec]:
    return [
        ToolSpec(
            name="list_skills",
            description="List internal Octopal skills from registry and auto-discovered bundles. Skills are internal tools, not MCP servers.",
            parameters={
                "type": "object",
                "properties": {
                    "include_disabled": {
        ...
        ToolSpec(
            name="use_skill",
            description="Read guidance from an internal Octopal skill by id. Prefer this generic tool for workers instead of hardcoding dynamic skill_<id> tools.",
            parameters={
                "type": "object",
                "properties": {
...
def _workspace_root() -> Path:
    raw = os.getenv("OCTOPAL_WORKSPACE_DIR", "").strip()
    if raw:
...
def _missing_config_requirements(value: Any) -> list[str]:
    missing: list[str] = []
    for item in _normalize_string_list(value):
        env_key = f"OCTOPAL_SKILL_CONFIG_{item.upper()}"
