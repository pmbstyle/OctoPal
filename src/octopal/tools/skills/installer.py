...
from octopal.tools.skills.bundles import SkillBundle, load_skill_bundle
from octopal.tools.skills.management import ensure_skills_layout
from octopal.tools.skills.runtime_envs import get_skill_env_status, prepare_skill_env
from octopal.tools.skills.scanner import scan_skill_bundle
...
def install_skill_from_source(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="octopal-skill-install-") as temp_dir_raw:
    ...
    for item in installs:
        ...
        if trusted and bool(item.get("has_scripts", False)):
            if scan_status == "review_required" and not force:
                raise ValueError(
                    f"skill '{skill_id}' has script scan findings; run `octopal skill verify {skill_id}` and re-run trust with --force after review"
                )
            if not scan_status:
                raise ValueError(
                    f"skill '{skill_id}' has not been verified yet; run `octopal skill verify {skill_id}` first"
                )
